...
const getProviderDisplayLabel = (
  ...
  if (providerId === "local_llama") return "Offline cleanup";
  if (providerId === APPLE_PROVIDER_ID) return "On-device cleanup";
  if (providerId === "custom") return "Custom connection";
  return `Cleanup connection ${index + 1}`;
};
...
export const usePostProcessProviderState = (): PostProcessProviderState => {
  ...
  const providerOptions = useMemo<DropdownOption[]>(() => {
    return providers
      ...
      .map((provider, index) => ({
        value: provider.id,
        label: getProviderDisplayLabel(provider.id, index),
      }));
  }, [providers]);
  ...
  const modelOptions = useMemo<ModelOption[]>(() => {
    ...
    const upsert = (value: string | null | undefined) => {
      ...
      options.push({
        value: trimmed,
        label: `Cleanup option ${options.length + 1}`,
      });
    };
