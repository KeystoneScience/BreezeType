...
export const useModels = () => {
  ...
  const loadModels = async () => {
    try {
      ...
      if (result.status === "ok") {
        ...
        setError("BreezeType could not finish getting ready. Please try again.");
      }
    } catch (err) {
  ...
  const downloadModel = async (modelId: string) => {
    try {
      ...
      if (result.status === "ok") {
        ...
        setError("BreezeType could not finish setup. Please try again.");
  ...
  const deleteModel = async (modelId: string) => {
    try {
      ...
      if (result.status === "ok") {
        ...
        setError("BreezeType could not remove that download. Please try again.");
        return false;
      }
    } catch (err) {
  ...
  useEffect(() => {
    ...
    const extractionFailedUnlisten = listen<{
      ...
      console.error("Failed to extract model:", event.payload.error);
      setError("BreezeType could not finish setup. Please try again.");
    });
