...
const normalizeDictionaryEntries = (
...
const dictionaryEntriesToWords = (entries: DictionaryEntry[] = []): string[] =>
  entries.map((entry) => entry.term.trim()).filter(Boolean);

const mergeDictionaryEntries = (
  words: string[] = [],
  ...
  return words
    ...
    .map((term) => ({
      term,
      definition: byTerm.get(term.toLowerCase()) ?? "",
    }));
};
...
const settingUpdaters: {
  ...
  fn_key_ptt_enabled: (value) =>
    commands.changeFnKeyPttSetting(value as boolean),
  ...
  meeting_detection_enabled: (value) =>
    commands.changeMeetingDetectionSetting(value as boolean),
  debug_mode: (value) => commands.changeDebugModeSetting(value as boolean),
  custom_words: (value) => commands.updateCustomWords(value as string[]),
  custom_dictionary: (value) =>
    commands.updateCustomDictionary(value as DictionaryEntry[]),
  word_correction_threshold: (value) =>
    commands.changeWordCorrectionThresholdSetting(value as number),
  paste_method: (value) => commands.changePasteMethodSetting(value as string),
  clipboard_handling: (value) =>
    commands.changeClipboardHandlingSetting(value as string),
  clipboard_quick_pastes: (value) =>
    commands.changeClipboardQuickPastesSetting(
      value as Partial<{ [key: string]: string }>,
    ),
  history_limit: (value) => commands.updateHistoryLimit(value as number),
  post_process_enabled: (value) =>
    commands.changePostProcessEnabledSetting(value as boolean),
  post_process_remove_fillers: (value) =>
    commands.changePostProcessRemoveFillersSetting(value as boolean),
  post_process_selected_prompt_id: (value) =>
    commands.setPostProcessSelectedPrompt(value as string),
  mute_while_recording: (value) =>
    commands.changeMuteWhileRecordingSetting(value as boolean),
  recording_output_mode: (value) =>
    commands.changeRecordingOutputModeSetting(value as string),
  recording_duck_level: (value) =>
    commands.changeRecordingDuckLevelSetting(value as number),
...
export const useSettingsStore = create<SettingsStore>()(
  subscribeWithSelector((set, get) => ({
      ...
      try {
        ...
        if (result.status === "ok") {
          ...
          const normalizedSettings: Settings = {
            ...
            clipboard_quick_pastes: settings.clipboard_quick_pastes ?? {},
            meeting_detection_enabled:
              settings.meeting_detection_enabled ?? true,
            recording_output_mode:
              settings.recording_output_mode ??
              (settings.mute_while_recording ? "mute" : "off"),
            recording_duck_level: settings.recording_duck_level ?? 0.09,
            post_process_remove_fillers:
              settings.post_process_remove_fillers ?? true,
          };
    ...
    updateSetting: async <K extends keyof Settings>(
      ...
      let nextValue: Settings[K] = value;
      let settingsPatch: Partial<Settings> = { [key]: value } as Partial<Settings>;

      setUpdating(updateKey, true);

      try {
        if (key === "custom_dictionary") {
          const normalized = normalizeDictionaryEntries(
            value as DictionaryEntry[],
          );
          const nextWords = dictionaryEntriesToWords(normalized);
          nextValue = normalized as Settings[K];
          settingsPatch = {
            custom_dictionary: normalized,
            custom_words: nextWords,
          };
        }

        if (key === "custom_words") {
          const trimmedWords = (value as string[])
            .map((word) => word.trim())
            .filter(Boolean);
          const mergedDictionary = mergeDictionaryEntries(
            trimmedWords,
            settings?.custom_dictionary ?? [],
          );
          nextValue = trimmedWords as Settings[K];
          settingsPatch = {
            custom_words: trimmedWords,
            custom_dictionary: mergedDictionary,
          };
        }
    ...
    updateBinding: async (id, binding) => {
      ...
      try {
        ...
        if (result.status === "error") {
          throw new Error(String(result.error ?? "Failed to set shortcut"));
        }
        if (!result.data.success) {
          throw new Error(result.data.error ?? "Failed to register shortcut");
        }
      } catch (error) {
