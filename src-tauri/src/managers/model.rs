...
impl ModelManager {
    pub fn new(app_handle: &AppHandle) -> Self {
        // Create models directory in app data (or fall back to a temp dir).
        let models_dir = match app_handle.path().app_data_dir() {
            Ok(dir) => dir.join("models"),
            Err(err) => {
                warn!(
                    "Failed to resolve app data dir for models: {}. Falling back to temp directory.",
                    err
                );
                std::env::temp_dir().join("breezetype").join("models")
            }
        };

        if let Err(err) = fs::create_dir_all(&models_dir) {
            warn!(
                "Failed to create models directory '{}': {}. Model downloads may fail.",
                models_dir.display(),
                err
            );
        }
        ...
        // Best-effort initialization: never crash the app on startup.
        if let Err(err) = manager.migrate_bundled_models() {
            warn!("Failed to migrate bundled models: {}", err);
        }
        if let Err(err) = manager.update_download_status() {
            warn!("Failed to update model download status: {}", err);
        }
        if let Err(err) = manager.auto_select_model_if_needed() {
            warn!("Failed to auto-select default model: {}", err);
        }
    ...
    fn migrate_bundled_models(&self) -> Result<()> {
        // Release builds ship transcription models inside Tauri resources.
        // `update_download_status` and `get_model_path` read them in place.
        Ok(())
    }
        ...
        for model in models.values_mut() {
            if model.is_directory {
                ...
                if !model.is_downloaded {
                    if let Some(bundled_dir) = bundled_model_path(&self.app_handle, &model.filename)
                    {
                        model.is_downloaded = bundled_dir.exists() && bundled_dir.is_dir();
                    }
                }
                ...
                if !model.is_downloaded {
                    if let Some(bundled_file) =
                        bundled_model_path(&self.app_handle, &model.filename)
                    {
                        model.is_downloaded = bundled_file.exists();
                    }
                }
                model.is_downloading = false;
    ...
    pub fn get_model_path(&self, model_id: &str) -> Result<PathBuf> {
        ...
        if model_info.is_directory {
            ...
            if model_path.exists() && model_path.is_dir() && !partial_path.exists() {
                ...
                if let Some(bundled_path) =
                    bundled_model_path(&self.app_handle, &model_info.filename)
                {
...
fn bundled_model_path(app_handle: &AppHandle, filename: &str) -> Option<PathBuf> {
    app_handle
        .path()
        .resolve(
