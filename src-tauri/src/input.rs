...
/// Option allows late initialization after permissions are granted.
...
pub fn create_enigo() -> Result<Enigo, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        // Keep permission prompts explicit (PermissionsGate / Help),
        // not implicit from background initialization.
...
pub fn get_cursor_position(app_handle: &AppHandle) -> Option<(i32, i32)> {
    ...
    if enigo.is_none() {
        if let Ok(new_enigo) = create_enigo() {
...
/// Sends backspace keystrokes to delete the most recently inserted text.
